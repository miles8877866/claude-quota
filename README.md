...
| `cu` | 用量估值（ccusage 風格）：**預設本月**，各模型 + 各帳號「等值美元」（需 python） |
| `cu --daily` | 每日明細（近 30 天，分帳號） |
| `cu --monthly` | 每月明細（分帳號） |
| `cu --all` | 有史以來全部 |
...
> 訂閱是月費吃到飽，這個金額是「若照 API 單價計費的等值」估算，方便比較各帳號/各模型用量。**預設只算本月、且分帳號**。`cu --daily`（每日明細）、`cu --monthly`（每月明細）、`cu --all`（全部歷史）、`cu --total-only`（只看總計）、`cu --watch`（持續監控）。
>
> 單價**偶爾自動抓線上**（同 ccusage 用的 LiteLLM 價格庫，含精確快取讀寫單價），快取 7 天才重抓一次；抓不到或離線時退回內建後備表。footer 會標「定價來源: 線上@日期 / 後備表」。`cu --refresh-prices` 可強制立即重抓。
...
**本工具全程在你本機運作，不會洩漏帳號資料。**

- **唯一對外連線**：`cu` 偶爾去 GitHub 下載一份**公開的 LiteLLM 價格表**（單向下載價格數字）。**不上傳**你的任何東西 —— 沒有帳號、token、用量或對話內容。`cq` 只呼叫 Anthropic 官方額度 API。
- **輸出不含敏感資訊**：只顯示本機目錄標籤（`claude1`…）與數字，**沒有 token、密鑰、email、對話內容**。
- **repo 不含個資**：本專案只有程式碼；你的 log、憑證、價格快取都在 `~/.claude/`（已被 `.gitignore` 排除），不會進 repo。別人 clone 只拿到工具，拿不到你的帳號。
- 別人裝了也只會看到**他們自己**電腦上的帳號 —— 工具是通用的。

### cu 與 cq 的資料範圍不同（重要）

| | 資料來源 | 範圍 |
|---|---|---|
| **cu**（用量估值） | 本機 log 檔 | **只算這台電腦跑過的** |
| **cq**（剩餘額度 %） | Anthropic 伺服器 | **整個帳號、所有裝置加總** |

- 同一帳號在多台機器用過：`cu` 在某台只看到該台的用量；`cq` 反映全部裝置加總的真實消耗。
- Claude Code 會定期清理本機舊 log，被清掉的紀錄 `cu` 會算不到（很舊的用量可能少算）。
